...
variable "fgt_password" {
  ...
  sensitive   = true
  description = <<-EOF
  Password for all FGTs (username is admin). It must be at least 8 characters long if specified.
...
variable "image_type" {
  ...
  Use the following command to check all FGT image types:
  ...
  description = <<-EOF
    ...
    Options:
        ...
        - internal_lb   : (Optional | object) If "internal_lb" is specified, an internal load balancer will be created in the "subnet_name" subnet.
            ...
            - ip_range_route_to_lb : (Optional | string | default:"") If "ip_range_route_to_lb" is specified, a route will be created in the VPC network.
    ...
    network_interfaces = [                     # Network interface of your FortiGate
      # Port 1 of your FortiGate. For this interface, this project creates an internal load balancer (ILB) and a route to the ILB.
      {
